...
with open("{}.txt".format(file_name),'a') as f:
                f.write('Iniciado em: {}\n\n'.format(datetime.now()))
                f.write("{0:^12}{1:^12}{2:^12}{3:^12}{4:^12}\n\n".format("X","Y", "Z", "ENCODER_A", "ENCODER_C"))
...
try:
    ...
    for _ in range(ciclos):
        ...
        for i in range(4):
            ...
            if (i==1): time.sleep(28)
            if (i==3): time.sleep(14.5)
            if (i!= 2):
                #### Lê display 1 ####
                ...
                encoder_C = caget("IMAS2:delta-test:AxisCActualPosition-Mon", as_string=True, timeout=.5)
                with open("{}.txt".format(file_name),'a') as f:
                    f.write('{}\t{}\t{}\t{}\t{}\n'.format(val1, val2, val3, encoder_A, encoder_C))
                    print('{0:^12}{1:^12}{2:^12}{3:^12}{4:^12}'.format(val1, val2, val3, encoder_A, encoder_C))
